...
static int find_cmd(unsigned char op, unsigned char masks[2])
{
    size_t i;
...
int gzip_compress(const void* src, const unsigned int src_len, void** dst, unsigned int* dst_len)
{
    ...
    *(unsigned int*)ptr = htonl(src_len);
    ...
    stream.next_out  = ptr;
    stream.avail_out = (unsigned int)dlen - sizeof(unsigned int);
    deflate(&stream, Z_FINISH);
    deflateEnd(&stream);
    *dst_len = (unsigned int)dlen - sizeof(unsigned int) - stream.avail_out;
...
msg_t* new_login_msg(unsigned int ip, unsigned int gateway, unsigned char mask, unsigned char request)
{
    ...
    ret->pfx         = (unsigned char)little16((uint16_t)(dst_len % 16));
...
{
    unsigned short src_len = (unsigned short)msg_data_length(input);
