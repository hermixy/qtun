...
ssize_t write_c(client_t* client, const void* buf, size_t count)
{
    if (this.use_udp)
    {
        return sendto(this.remotefd, buf, count, 0, (struct sockaddr*)&client->addr, sizeof(client->addr));
    }
    else
    {
        const char* ptr = buf;
        ...
        while (left)
        {
            ssize_t written = write(client->fd, ptr, left);
...
inline ssize_t udp_read(int fd, void* buf, size_t count, struct sockaddr_in* srcaddr, socklen_t* addrlen)
{
    return recvfrom(fd, buf, count, 0, (struct sockaddr*)srcaddr, addrlen);
}
